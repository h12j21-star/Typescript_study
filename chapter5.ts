...
let y; //변수만 만들면 any 자동할당

// 1. return
function 함수return(x: number): number {
  return x * 2;
}
// 에러 : 함수return('6');
...
function ch5Hw1(name?: string): void {
...
//숙제2
//숫자, 문자넣을때
function ch5Hw2(str: number | string): number {
  return str.toString().length;
}
//숙제3
//함수 파라미터 : 월소득:숫자,집보유여부:boolean,매력점수 : string
// 할당을 해주면 타입지정 안해도 됨
function ch5Hw3(소득: number, 집보유: boolean, 매력점수: string): string {
  let score: number = 0;
  if (집보유) {
    score += 500;
  }
